...
class WatchLaterShow extends React.Component {

  _displayDetailForFilm = (idFilm) => {
  //  console.log("Display film with id " + idFilm)
  //  getFilmCreditsFromApi(idFilm).then(data => console.log(data));
     this.props.navigation.navigate("ShowDetail", { idShow: idFilm })
  }
...
for (let i = 0; i < highestTimeoutId; i++) {
    clearTimeout(i);
              ...
              const user = firebase.auth().currentUser
              var ref = firebase.database().ref('users/' + user.uid+'/WatchListShows'); //Here assuming 'Users' as main table of contents
...
/*  for (let i = 0; i < this.state.arrData.length; i++){
  ...
  renderItem={({item}) => <ShowItem film={item} displayDetailForFilm={this._displayDetailForFilm}/>}
/>
              )
              }
          ...
          <Header style={{ marginTop: 24, backgroundColor: '#fff',   height: 50 }} >
            <Left  style={{marginRight: 15, flexDirection:'row', flex:1}} >
                <Icon name='md-menu' style={{marginRight: 20}} onPress={()  => this.props.navigation.openDrawer()} />
                  <Text style={{marginRight: 0, fontSize:20,fontWeight:'bold'}}>À voir - Série</Text>
            </Left>
            <Body>
